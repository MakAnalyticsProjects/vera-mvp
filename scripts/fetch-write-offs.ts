  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        ...
        // Detection logic lives in `shared/domain/write-offs` so the DB read
        // path uses the same function. `toWriteOffRecord` returns null when
        // the payload has no Amount Withheld discount or fails parsing.
        const record = toWriteOffRecord(job, body);
        if (record) {
