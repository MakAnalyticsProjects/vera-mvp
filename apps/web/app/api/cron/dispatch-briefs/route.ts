  ...
  for (const sch of dueBackfills) {
    ...
    try {
      ...
      await recordAudit(db, {
        ...
        summary: `Scheduled ${sch.source === 'rooflink_jobs' ? 'Rooflink jobs' : 'Rooflink estimate line items'} ${mode} backfill started`,
        details: {
          source: sch.source,
          mode,
          runId: run.id,
          scheduleId: sch.id,
          syncedSince: sch.lastSyncedAt?.toISOString() ?? null,
          trigger: 'scheduled',
        },
      });
      backfillKickoffs.push({
